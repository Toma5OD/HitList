...
object FirebaseImageManager {
    ...
    fun checkStorageForExistingProfilePic(userid: String) {
        ...
        imageRef.metadata.addOnSuccessListener { //File Exists
            imageRef.downloadUrl.addOnCompleteListener { target ->
                imageUri.value = target.result!!
            }
            //File Doesn't Exist
    ...
    fun uploadImageToFirebase(userid: String, bitmap: Bitmap, updating : Boolean) {
        ...
        imageRef.metadata.addOnSuccessListener { //File Exists
            if(updating) // Update the existing image
            {
                uploadTask = imageRef.putBytes(data)
                uploadTask.addOnSuccessListener { ut ->
                    ut.metadata!!.reference!!.downloadUrl.addOnCompleteListener { target ->
                        imageUri.value = target.result!!
