...
3. Nav Controller <br>
Each nav host fragment has an instance of nav controller. Using the navigation method of the navController class willl perform the navigation based on the destination id given in the nav_graph. Code snippet from ~ui/list/HitListFragment.onCreateView
...
- Swipe-to-Delete <br>
   When a user swipes left it triggers a callback function *SwipeToDeleteCallback* located in the utils package. The swipe feature is handled in the HitList fragment. Using a *tag* property of the view and tagging it with the target 'id' -  the 'id' is retrieved removes the hit target from both the recyclerView and the server.
...
- Swipe-to-Edit <br>
   Similar to how the delete functions, when a user swipes right a callback to *SwipeToEditCallback* in the utils package. This callback navigates the user to the Hit Edit Fragment. 
   ```
